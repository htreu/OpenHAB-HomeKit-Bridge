...
import { LightbulbItem } from './subtypes/LightbulbItem.js';
import { OutletItem } from './subtypes/OutletItem.js';
...
class Switch {
  constructor(name, url, state, itemType) {
    ...
    if ('GroupItem' === itemType) {
      ...
      new RestClient().fetchItem(url, function (error, item) {
  ...
  buildItemType(itemType) {
    ...
    case 'RollershutterItem':
      ...
      break;
    case 'OutletItem':
      return new OutletItem(this.name, this.url, this.state);
      break;
    default:
      return new LightbulbItem(this.name, this.url, this.state);
    }
  };
}
