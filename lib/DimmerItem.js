...
class DimmerItem {
  ...
  updateOpenHabItem(value, callback) {
    ...
    console.log("read power state for " + this.name + " from " + this.url);
    ...
    console.log("read brightness state for " + this.name + " from " + this.url);
		request(this.url + '/state?type=json', function (error, response, body) {
		  if (!error && response.statusCode == 200) {
        console.log("read brightness state: '" + body + "' for " + this.name + " from " + this.url);
		    callback(+body);
		  }
		})
	}
