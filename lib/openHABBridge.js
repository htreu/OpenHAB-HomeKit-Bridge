...
import { RollershutterAccessoryControllerFactory } from './RollershutterAccessoryControllerFactory.js';
...
function publishOpenHABBridgeAccessory(openHABWidgets) {
  ...
  for (var i = 0; i < openHABWidgets.length; i++) {
    ...
    if (openHABWidget.type === itemProvider.SWITCH_ITEM) {
      accessoryController = new SwitchAccessoryControllerFactory()
        .createSwitchAccessoryController(openHABWidget);
    }
    if (openHABWidget.type === itemProvider.DIMMER_ITEM) {
      accessoryController = new DimmerAccessoryControllerFactory()
        .createDimmerAccessoryController(openHABWidget);
    }
    if (openHABWidget.type === itemProvider.COLOR_ITEM) {
      accessoryController = new ColorAccessoryControllerFactory()
        .createColorAccessoryController(openHABWidget);
    }
    if (openHABWidget.type === itemProvider.ROLLERSHUTTER_ITEM) {
      accessoryController = new RollershutterAccessoryControllerFactory()
        .createRollershutterAccessoryController(openHABWidget);
    }
