...
class ColorItem {
  ...
  buildAccessory(state) {
    let accessory = new HAPNodeJS.Accessory(
      this.name, HAPNodeJS.uuid.generate(this.name));
    ...
    let service = accessory.addService(HAPNodeJS.Service.Lightbulb, this.name);

    let charactersiticOnOff =
      service.getCharacteristic(HAPNodeJS.Characteristic.On);
    ...
    let charactersiticBrightness =
      service.addCharacteristic(HAPNodeJS.Characteristic.Brightness);
    ...
    let charactersiticHue =
      service.addCharacteristic(HAPNodeJS.Characteristic.Hue);
    ...
    let charactersiticSaturation =
      service.addCharacteristic(HAPNodeJS.Characteristic.Saturation);
  ...
  getCurrentStateFromOpenHAB(callback) {
    ...
    var _this = this;
      ...
      // set hue
      _this.getCharacteristic(HAPNodeJS.Characteristic.Hue)
        .setValue(hue,
          function() { // callback to signal us iOS did process the update
            finished++;
            if (finished == 4) {
      ...
      // set saturation
      _this.getCharacteristic(HAPNodeJS.Characteristic.Saturation)
        .setValue(saturation,
          function() { // callback to signal us iOS did process the update
            finished++;
            if (finished == 4) {
