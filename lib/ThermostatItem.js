...
class ThermostatItem extends OhItem {
      ...
      if (this.isItemTemperature(item)) {
        if (this.isItemSetting(item)) {
          ...
          if (this.isCelsius(item)) {
            ...
            this.temperatureDisplayUnitsCharacteristic.setValue(1);
  ...
  updateCharacteristicValue(item, characteristic, message) {
    ...
    let convertToCelsius = this.shouldConvertToCelsius(item);
  ...
  setCharacteristicValue(item, value, callback) {
    ...
    request.post(
        this.url,
        {
          body: ""+this.convertState(value, convertToCelsius),
  ...
  convertState(state, convertToCelsius) {
    ...
    if (convertToCelsius) {
      state = (state - 32) * (5/9);
      state = state.toFixed(1); //will lose some precision here
    }
