...
import debug from 'debug'; let logger = debug('Slider');
...
class Slider {
  ...
  updateOpenHabItem(value, callback) {
    ...
    if (process.env.NODE_ENV !== 'test') {
      logger('slider value from iOS: ' + value + ' for ' + this.name + ', sending command to openhab: ' + command + '');
    }
  ...
  updateCharacteristics(message) {
    ...
    /* istanbul ignore next */
    if (process.env.NODE_ENV !== 'test') {
      logger('slider value from openHAB: ' + message + ' for ' + this.name + ', updating iOS: ' + brightness + '');
    }
