...
class ItemProvider {
  ...
  createHomeKitAccessories(openhabItems) {
    ...
    for (var i = 0; i < openhabItems.length; i++) {
      ...
      if (openHABWidget.type === itemType.NUMBER_ITEM) {
      ...
      if (openHABWidget.type == itemType.GROUP_ITEM) {
        if (openHABWidget.tags.indexOf("thermostat" > -1)) {//thermostat type
          accessory = new ThermostatItem(
              openHABWidget.name, openHABWidget.link, openHABWidget.state, this.ohVersion, openHABWidget.groupItems).accessory;
        } else {
