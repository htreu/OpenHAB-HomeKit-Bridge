...
import debug from 'debug'; let logger = debug('OutletItem');
...
class OutletItem {
  ...
  buildAccessory(state) {
    ...
    let charactersiticOnOff = accessory
      .addService(Service.Outlet, this.name)
  ...
  updateCharacteristics(message) {
    ...
    if (process.env.NODE_ENV !== 'test') {
      logger('outlet value from openHAB: ' + message + ' for ' + this.name + ', updating iOS: ' + command);
    }
  ...
  updateOpenHabItem(value, callback) {
    ...
    /* istanbul ignore next */
    if (process.env.NODE_ENV !== 'test') {
      logger('outlet value from iOS: ' + value + ' for ' + this.name
        + ', sending command to openhab: ' + command);
    }
    ...
    request.post(
        ...
        function (error, response, body) {
            callback(); // we are done updating the Outlet item in OpenHAB
        }
    );
  };
}
