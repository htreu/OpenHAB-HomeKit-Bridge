    ...
    for (var i = 0; i < openhabItems.length; i++) {
      ...
      if (openHABWidget.type === itemType.SWITCH_ITEM) {
        accessory = new SwitchItem(
          openHABWidget.name, openHABWidget.itemName, openHABWidget.link, openHABWidget.state, this.ohVersion).accessory;
      }
        ...
        if (openHABWidget.displayPattern) {
          ...
          if (unitOfMeasurement == "C") { //temperature in Celsius
            accessory = new TemperatureSensor(
                openHABWidget.name, openHABWidget.itemName, openHABWidget.link, openHABWidget.state, this.ohVersion, false).accessory;
          } else if (unitOfMeasurement == "F") { //temperature in Fahrenheit
            accessory = new TemperatureSensor(
                openHABWidget.name, openHABWidget.itemName, openHABWidget.link, openHABWidget.state, this.ohVersion, true).accessory;
